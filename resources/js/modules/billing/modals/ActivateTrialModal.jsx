...
export default function ActivateTrialModal({ open, tenants, onClose, onActivated }) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            if (isBulk) {
                ...
                toast.success(`${tenants.length} tenant(s) activated and assigned to new plan`);
            } else {
                await api.put(`/admin/api/tenants/${singleTenant.id}`, {
                    plan_id: selectedPlanId,
                });
                toast.success('Tenant activated and assigned to new plan');
            }
            onActivated?.();
            onClose();
        } catch (err) {
            ...
            <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pb: 1 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 600 }}>
                        {step === 'confirm' ? 'Confirm Activation' : 'Activate & Assign Plan'}
                    </Typography>
                </Box>
                <Tooltip title="Close">
            ...
            <DialogContent>
                {singleTenant && (
                    <Typography variant="body2" sx={{ mb: 2, color: '#64748b' }}>
                        Activating <strong>{singleTenant.name}</strong> from{' '}
                        <Chip label="Trial" size="small" sx={{ bgcolor: '#fef9c3', color: '#854d0e', fontSize: 11, height: 20 }} />
                        {' → '}
                        <Chip label="Active" size="small" sx={{ bgcolor: '#dcfce7', color: '#166534', fontSize: 11, height: 20, fontWeight: 600 }} />
                    </Typography>
                )}
                {isBulk && (
                    <Typography variant="body2" sx={{ mb: 2, color: '#64748b' }}>
                        Activating <strong>{tenants.length} tenant(s)</strong> from Trial to Active
                    </Typography>
                )}
                                    ...
                                    sx={{
                                        ...
                                        border: isSelected
                                            ...
                                            : isCurrent
                                                ? '2px solid #e2e8f0'
                                                : '1px solid #e2e8f0',
                                        bgcolor: isSelected ? '#f5f3ff' : isCurrent ? '#f8fafc' : '#ffffff',
                            ...
                            {isBulk ? (
                                <>
                                    <Typography variant="body2" sx={{ mt: 0.5 }}>
                                        Activate <strong>{tenants.length} tenant(s)</strong> and assign <strong>{selectedPlan.name}</strong>?
                                    </Typography>
                                ...
                                <Typography variant="body2" sx={{ mt: 1 }}>
                                    Activate <strong>{singleTenant?.name}</strong> and assign plan{' '}
                ...
                {step === 'confirm' ? (
                    ...
                    <>
                        ...
                        <Button
                            ...
                            sx={{ fontWeight: 600, bgcolor: '#8b5cf6', '&:hover': { bgcolor: '#7c3aed' } }}
                        >
                            Continue
                        </Button>
                    </>
