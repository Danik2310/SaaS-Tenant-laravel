...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | JWT Per-Guard Secrets
    |--------------------------------------------------------------------------
    |
    | Signing secrets keyed by guard name. Each guard resolves its own JWT
    | provider so a token minted for one context (e.g. tenant users via the
    | "web" guard) cannot be validated by another context (e.g. central
    | admins via the "admin" guard) even if it reaches that guard.
    |
    | Falls back to the shared `secret` when a per-guard secret is unset,
    | preserving existing tokens during rollout.
    |
    */

    'guard_secrets' => [
        'web' => env('JWT_SECRET'),
        'admin' => env('JWT_ADMIN_SECRET', env('JWT_SECRET')),
    ],
